...
description: >-
  ...
  when the user says "/architect", asks to plan or design a module, feature,
...
`/architect plan <idea>` · `/architect review <target>` — bare or ambiguous
invocation: ask which mode and what target.
...
convention (`skills/shared/durable-docs.md`) — a design plan (PLAN) or graded
...
the path. Before declaring it done, pressure-test the draft: the `advisor`
for the design tradeoffs (it reasons with the transcript), a fresh `review`
worker for grounding against the repo and sources — fold the findings from
both.

On a longer arc, keep the running state on a board file per
`skills/shared/board-files.md`.
